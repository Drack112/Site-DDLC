typeof navigator === "object" &&
	(function (global, factory) {
		...
		function _createClass$1(e, t, n) {
			return (
				t && _defineProperties$1(e.prototype, t), n && _defineProperties$1(e, n), e
			)
		}
		...
		var RangeTouch = (function () {
			function e(t, n) {
				_classCallCheck$1(this, e),
					is.element(t)
						? (this.element = t)
						: is.string(t) && (this.element = document.querySelector(t)),
					is.element(this.element) &&
						is.empty(this.element.rangeTouch) &&
						((this.config = _objectSpread2$1({}, defaults, {}, n)), this.init())
			}
					...
					[
						...
						},
						{
							...
							value: function value(t) {
								...
								return (
									0 > (n = (100 / c.width) * (i.clientX - c.left))
										? (n = 0)
										: 100 < n && (n = 100),
					...
					[
						{
							...
							value: function value(t) {
								var n =
									...
									r = null
								if (
									(is.empty(t) || is.string(t)
										? (r = Array.from(
												document.querySelectorAll(is.string(t) ? t : 'input[type="range"]')
										  ))
										: is.element(t)
										? (r = [t])
										: is.nodeList(t)
										? (r = Array.from(t))
										: is.array(t) && (r = t.filter(is.element)),
									is.empty(r))
								)
									return null
		...
		var controls = {
			...
			createButton: function createButton(buttonType, attr) {
				...
				extend(
					attributes,
					getAttributesFromSelector(this.config.selectors.buttons[type], attributes)
				)
			...
			timeUpdate: function timeUpdate(event) {
				...
				var invert =
					!is$1.element(this.elements.display.duration) && this.config.invertTime // Duration
          ...
          emptyElement(list);
           options.forEach(option => {
			...
			setSpeedMenu: function setSpeedMenu() {
				...
				var type = "speed"
				var list =
					this.elements.settings.panels.speed.querySelector('[role="menu"]') // Filter out invalid speeds
			...
			create: function create(data) {
				...
				dedupe(
					is$1.array(this.config.controls) ? this.config.controls : []
					...
					if (control === "settings" && !is$1.empty(_this10.config.settings)) {
						...
						_this10.config.settings.forEach(function (type) {
							...
							var menuItem = createElement(
								"button",
								extend(
									getAttributesFromSelector(_this10.config.selectors.buttons.settings),
									{
										type: "button",
										class: ""
		...
		var Fullscreen = /*#__PURE__*/ (function () {
			function Fullscreen(player) {
				...
				this.player.elements.fullscreen =
					player.config.fullscreen.container &&
					closest(this.player.elements.container, player.config.fullscreen.container) // Register event listeners
					...
					},
					{
						...
						get: function get() {
							...
							return element && element.shadowRoot
								? element === this.target.getRootNode().host
								: element === this.target
						}, // Get target element
					},
					{
		...
		var ui = {
			...
			build: function build() {
				...
				if (!this.supported.ui) {
					this.debug.warn(
						"Basic support only for ".concat(this.provider, " ").concat(this.type)
			...
			migrateStyles: function migrateStyles() {
				...
				Object.values(_objectSpread2({}, this.media.style)) // We're only fussed about Plyr specific properties
					...
					.forEach(function (key) {
						...
						_this5.elements.container.style.setProperty(
							key,
							_this5.media.style.getPropertyValue(key)
				...
				},
				{
					...
					value: function controls$1() {
						...
						this.bind(
							...
							function (event) {
								elements.controls.pressed = ["mousedown", "touchstart"].includes(
									event.type
								)
							}
		...
		var vimeo = {
			...
			ready: function ready() {
				...
				var premium = config.premium,
					...
					frameParams = _objectWithoutProperties(config, [
						"premium",
						"referrerPolicy",
				...
				},
				{
					...
					value: function pollCountdown() {
						...
						var update = function update() {
							...
							var label = ""
								.concat(i18n.get("advertisement", _this5.player.config), " - ")
								.concat(time)
				...
				},
				{
					...
					value: function showImageAtCurrentTime() {
						...
						var thumbNum = this.thumbnails[0].frames.findIndex(function (frame) {
							return (
								_this6.seekTime >= frame.startTime && _this6.seekTime <= frame.endTime
							)
						})
				...
				},
				{
					...
					value: function showImage(
						...
						this.preloadNearby(thumbNum, true)
							...
							.then(
								this.getHigherQuality(qualityIndex, previewImage, frame, thumbFilename)
							)
										...
										if (newThumbFilename !== oldThumbFilename) {
											...
											if (!_this9.loadedImages.includes(newThumbFilename)) {
												...
												_this9.player.debug.log(
													"Preloading thumb filename: ".concat(newThumbFilename)
												)
												...
												previewImage.onload = function () {
													_this9.player.debug.log(
														"Preloaded thumb filename: ".concat(newThumbFilename)
													)
								...
								setTimeout(function () {
									...
									if (_this10.showingThumbFilename === thumbFilename) {
										_this10.player.debug.log(
											"Showing higher quality thumb for: ".concat(thumbFilename)
										)
				...
				},
				{
					...
					value: function toggleScrubbingContainer() {
						var toggle =
							arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false
						var className =
							this.player.config.classNames.previewThumbnails.scrubbingContainerShown
				...
				this.destroy.call(
					...
					function () {
						...
						var _sources = _slicedToArray(sources, 1),
							...
							provider =
								_sources$$provider === void 0 ? providers.html5 : _sources$$provider,
							src = _sources$.src
